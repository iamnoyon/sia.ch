    ...
    while url and office_url:
        ...
        while n<50:
            ...
            if indv_zip and office_indv_zip: #check for empty zip field
                ...
                if (df['ZIP_CODE'].eq(int(indv_zip))).any() and (df['ZIP_CODE'].eq(int(office_indv_zip))).any():
                ...
                #***implement later
                office_contact = indv_office_doc.xpath('//table//tr[4]/td[2]/div/text()')
                ...
                office_wdf = pd.DataFrame([[ids+1, indv_office_url_lang, office_indv_lang, join_indv_office_full_address_clean,indv_office_full_address_clean[0], indv_office_full_address_clean[1], indv_office_full_address_clean[2], '', '','','','',office_sector]], columns=["ID","URL", "LANGUGE", "FULL_ADDRESS", "NAME", "ADDRESS", "CITY", "ZIP_CODE", "EMAIL", "TEL", "FAX", "WEBSITE", "SECTOR"])
                #with pd.ExcelWriter("member.xlsx") as writer:
                    #wdf.to_excel(writer, sheet_name='member', index=False)
                #print(wdf)
                if fe_flag==1:
